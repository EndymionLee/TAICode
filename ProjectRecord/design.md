...
### 8. Web 模式 `src/web/` (v1.9)

`taicode --web` 启动 Fastify HTTP + WebSocket 服务（端口 3000），前端无需构建。

**架构**: EventBus 事件 → relay.ts 桥接 → WebSocket 广播 → 前端渲染。Runtime 完全无感知，Web 只是另一个订阅者。

**前端**: 单 HTML 文件，两栏布局——左侧可折叠文件面板（ls 浏览/代码编辑器），右侧对话区（流式渲染 + Execution log 折叠块 + 任务状态栏）。支持黑白主题切换、移动端侧滑抽屉、拖拽调整面板宽度。

**双向通信**: 前端 → WebSocket → server.ts → session.ask()。Events → relay.ts → WebSocket → 前端渲染。

**文件操作**: 点击文件弹出编辑器（Monospace 字体），文本类文件可编辑保存，自动刷新文件列表。
...
### 12. ReAct Agent `task/react-agent.ts` (v1.8)
...
| v1.9 | Web 模式: Fastify + WebSocket + 前端 (消息/状态/任务进度/文件面板/编辑器) |
